...
def generate_times_vs_theoretical(algorithm, df, density):
    ...
    time = df[f"{density}"].iloc[2]
    n = df["Input Size"].iloc[2]
    m = (df["Input Size"].iloc[2] ** 2) * density
    ...
    n_squared_log_n = time / ((n**2) * np.log2(n))
    theoretical_df["O(n^2 log n)"] = n_squared_log_n * ((n_theoretical**2) * np.log2(n_theoretical))

    # Calculates O(m + n log n + m log n) for improved kruskals
    m_plus_n_log_n_plus_m_log_n = time / (m + (n * np.log2(n)) + (m * np.log2(n)))
    theoretical_df["O(m + n log n + m log n)"] = m_plus_n_log_n_plus_m_log_n * (m_theoretical + (n_theoretical * np.log2(n_theoretical)) + (m * np.log2(n_theoretical)))
    ...
    if algorithm == 'naive_dijkstras':
        ...
        plt.plot(theoretical_df["Input Size"], theoretical_df["O(n^2 log n)"], '-', label=r"Theoretical $O(n^2 log n)$", color = 'b')
        plt.plot(theoretical_df["Input Size"], theoretical_df["Squared"], '-', label=r"Theoretical $O(n^2)$", color = 'r')
        plt.plot(theoretical_df["Input Size"], theoretical_df["Cubed"], '-', label=r"Theoretical $O(n^3)$", color = 'y')
    ...
    plt.title(f"Search Performance ({algorithm}, Density = {density})")
...
def generate_times_vs_theoretical_log(algorithm, df, density):
    plt.figure()
    ...
    theoretical_df['Input Size'] = [i for i in range(1, df["Input Size"].iloc[-1] + 5, 5)]  # Avoid log(0) by starting from 1
    ...
    # Calculates O(mn) for naive Kruskal's and Prim's
    n_plus_m_log_n = time / (n * m)
    theoretical_df["O(mn)"] = n_plus_m_log_n * (n_theoretical * m_theoretical)

    # Calculates O(m log n) for improved Prim's
    m_log_n = time / (m * np.log2(n))
    theoretical_df["O(m log n)"] = m_log_n * (m_theoretical * np.log2(n_theoretical))

    # Calculates O(m + n log n) for improved Kruskal's
    ...
    # n^4
    quartic = time / (n ** 4)
    theoretical_df["Quartic"] = quartic * (n_theoretical ** 4)

    # Plot different things based on algorithm
    if algorithm == 'naive_dijkstras':
        plt.loglog(theoretical_df["Input Size"], theoretical_df["Linearithmic"], '-', label=r"Theoretical $O(n \log n)$", color='b')
        plt.loglog(theoretical_df["Input Size"], theoretical_df["Squared"], '-', label=r"Theoretical $O(n^2)$", color='r')
        plt.loglog(theoretical_df["Input Size"], theoretical_df["Cubed"], '-', label=r"Theoretical $O(n^3)$", color='y')

    elif algorithm == 'heap_dijkstras':
        plt.loglog(theoretical_df["Input Size"], theoretical_df["O(m log n)"], '-', label=r"Theoretical $O(m \log n)$", color='b')
        plt.loglog(theoretical_df["Input Size"], theoretical_df["Squared"], '-', label=r"Theoretical $O(n^2)$", color='r')
        plt.loglog(theoretical_df["Input Size"], theoretical_df["Cubed"], '-', label=r"Theoretical $O(n^3)$", color='y')

    elif algorithm == 'naive_kruskals':
        plt.loglog(theoretical_df["Input Size"], theoretical_df["O(mn)"], '-', label=r"Theoretical $O(mn)$", color='b')
        plt.loglog(theoretical_df["Input Size"], theoretical_df["Squared"], '-', label=r"Theoretical $O(n^2)$", color='r')
        plt.loglog(theoretical_df["Input Size"], theoretical_df["Cubed"], '-', label=r"Theoretical $O(n^3)$", color='y')

    elif algorithm == 'improved_kruskals':
        plt.loglog(theoretical_df["Input Size"], theoretical_df["O(n^2 log n)"], '-', label=r"Theoretical $O(n^2 \log n)$", color='b')
        plt.loglog(theoretical_df["Input Size"], theoretical_df["Squared"], '-', label=r"Theoretical $O(n^2)$", color='r')
        plt.loglog(theoretical_df["Input Size"], theoretical_df["Cubed"], '-', label=r"Theoretical $O(n^3)$", color='y')
    ...
    # Measured Times
    plt.loglog(df["Input Size"], df[f"{density}"], 'o--', label="Measured Times", color='k')

    # Labels and Formatting
    plt.title(f"Search Performance ({algorithm}, Density = {density})")
    plt.xlabel("Input Size (log scale)")
    plt.ylabel("Time To Complete (log scale)")
    plt.xlim(df["Input Size"].iloc[0], df["Input Size"].iloc[-1] * 1.1)
    plt.legend()

    plt.grid(True, which="major", linestyle='--')  # Enable grid lines for both major and minor ticks

    # plt.show()
    plt.savefig(f"plots\graph_algorithms\{algorithm}\{algorithm}_theoretical_runtime_log_scale")   
...
def generate_graph_comparison_log(algorithms, density, alg_type):
    ...
    for algorithm, line_style in zip(algorithms, line_styles):
        ...
        # Plot the runtime data for the algorithm
        plt.loglog(grouped, label=legend, marker=line_style[0], linestyle=line_style[1], color=line_style[2])
